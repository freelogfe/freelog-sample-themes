<template>
  <div class="sign-list-wrapper">
    <!-- PC -->
    <div class="pc-sign-list-wrapper">
      <div class="title">签约记录</div>
      <template v-if="$store.state.signedList">
        <div class="sign-list" v-if="$store.state.signedList.length">
          <!-- <template v-for="item in $store.state.signedList" :key="item.exhibitId">
          <second-album class="list-item" :data="item" />
          <voice class="list-item" :data="item" statusShow authShow />
        </template> -->
          <voice
            class="list-item"
            :data="item"
            statusShow
            authShow
            v-for="item in $store.state.signedList"
            :key="item.exhibitId"
          />
        </div>
        <div class="no-data-tip" v-else>暂无任何签约记录</div>
      </template>
