...
<template>
  <div class="detail-wrapper" v-if="!loading">
      ...
      <div class="normal-exhibit" v-else-if="voiceInfo.articleInfo.status === 1">
        ...
        <div class="bottom-area" v-if="voiceInfo?.articleInfo.articleType === 3">
          <div class="tab-box">
            ...
            <div class="tab" :class="tab === 2 && 'active'" @click="changeTab(2)">歌单介绍</div>
          </div>
    ...
    <div class="pc-detail-wrapper" v-if="!store.inMobile">
      ...
      <!-- 专辑推荐 -->
      <div v-if="recommendData.length">
        <div class="recommend-list" v-if="voiceInfo?.articleInfo?.articleType === 3">
          <home-play-list hasHeader isRecommend :data="recommendData" />
        </div>
        <!-- 单曲推荐 -->
...
export default {
  ...
  methods: {
    ...
    async getRecommendList() {
      ...
      const isCollection =
        this.voiceInfo?.articleInfo?.articleType === 1 &&
        recommendData.some(item => item.articleInfo.articleType === 3);
      if (isCollection) {
